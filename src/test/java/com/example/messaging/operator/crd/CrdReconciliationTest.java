...
class CrdReconciliationTest {
     ...
     */
    static class OwnershipValidator {
        ...
        public ValidationResult validateDelete(Object resource, String requestingOwner) {
            ...
            if (!resourceOwner.equals(requestingOwner)) {
                return ValidationResult.invalid(
                        String.format("ApplicationService '%s' cannot delete resource owned by '%s'",
                                requestingOwner, resourceOwner));
            }
        ...
        private ValidationResult validateApplicationServiceExists(String appServiceName, String namespace) {
            ...
            if (appService == null) {
                return ValidationResult.invalid(
                        "Referenced ApplicationService '" + appServiceName + "' does not exist");
            }
            return ValidationResult.valid();
        }
        ...
        private ValidationResult validateVirtualClusterExists(String clusterRef, String namespace,
            ...
            if (!vc.getSpec().getApplicationServiceRef().equals(expectedAppService)) {
                return ValidationResult.invalid(
                        String.format("VirtualCluster '%s' is owned by '%s', not '%s'",
        ...
        private ValidationResult validateServiceAccountExists(String saRef, String namespace,
            ...
            if (!sa.getSpec().getApplicationServiceRef().equals(expectedAppService)) {
                return ValidationResult.invalid(
                        String.format("ServiceAccount '%s' is owned by '%s', not '%s'",
